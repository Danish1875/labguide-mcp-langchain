...
2. From the project root directory, start the MCP Inspector using `npx`:
   ...
   ```
   ![MCP Inspector start](../Screenshot/assets/lab03/task1.2.png)
...
3. The MCP Inspector opens in your browser. You will see a connection panel on the left. Configure the connection as follows:
   ...
   > **Your Burger MCP URL** is in your `.env` file as `BURGER_MCP_URL`. 
   
   You can also retrieve it quickly with:
   > ```powershell
   > azd env get-values | Select-String BURGER_MCP_URL
   > ```

   ![Burger-MCP URL](../Screenshot/assets/lab03/task1.3.1.png)
...
5. Click on **`get_burgers`** and then click **Run Tool**. You will see the raw JSON response containing all the burgers in your menu — the same data the chat agent uses when you ask "what's on the menu?".
   ...
   ![get_burgers result](../Screenshot/assets/lab03/task1.5.png)
...
7. Click on **`get_burger_by_id`**. In the input field, paste the burger ID you copied and click **Run Tool**. You should get the full details for that specific burger.
   ...
   ![get_burger_by_id result](../Screenshot/assets/lab03/task1.7.png)
...
8. Now test **`place_order`**. This tool requires two inputs:
   ...
   ![place_order input](../Screenshot/assets/lab03/task1.8.png)
...
10. Copy the `orderId` from the `place_order` response or you can also check with the agent. Click on **`get_orders_by_id`**, run it with the same `orderId` you used, and confirm your order appears in the list.

    ![get_orders result](../Screenshot/assets/lab03/task1.10.png)

11. Finally, test **`delete_order_by_id`** using the `orderId` and `userId`. 

   >You can fetch your unique **userId** from the agent web page. Run the tool and confirm the order is cancelled.

   ![delete_order_by_id result](../Screenshot/assets/lab03/task1.11.1.png)

   ![Order cancelled on dashboard](../Screenshot/assets/lab03/task1.11.2.png)
...
> **Congratulations** on completing Task 1! You have tested the MCP tools directly via the Inspector. In the next task, you will wire this same server into GitHub Copilot.
...
3. Inside the `.vscode` folder, create a new file named `mcp.json`:
   ...
   ![Create mcp.json](../Screenshot/assets/lab03/task2.3.png)

4. Paste the following configuration into `mcp.json`, replacing the URL with your live Burger MCP Server endpoint (you can find your url in the `.env` file as `BURGER_MCP_URL`):
   ...
   ![mcp.json configuration](../Screenshot/assets/lab03/task2.4.png)

   > **Your Burger MCP URL** is available in your `.env` file as well as retrievable with the following command:
   ```powershell
   azd env get-values | Select-String "BURGER_MCP_URL"
   ```
...
5. Once saved, VS Code will detect the MCP configuration. A **Start** button or a notification will appear at the top of the `mcp.json` file prompting you to activate the server. Click **Start**.
   ...
   ![Start MCP server in VS Code](../Screenshot/assets/lab03/task2.5.png)
...
6. Open the **GitHub Copilot Chat** panel from the VS Code sidebar (the chat bubble icon). At the top of the chat input, switch to **Agent Mode** by clicking the mode selector and choosing **Agent**.
   ...
   ![Switch to Agent Mode](../Screenshot/assets/lab03/task2.6.png)
...
7. In the chat input, click the **Tools** icon (plug/tool icon) to see the list of available tools. You should see **burger-mcp** listed with all 9 tools underneath it. Ensure it is enabled.
   ...
   ![Verify tools in Copilot](../Screenshot/assets/lab03/task2.7.png)

8. Now test the integration. Type the following in the Copilot chat and press **Enter**:

   `"which burger is the most expensive if ordered with extra cheese?"`

   Copilot will call the `get_burgers` and `get_toppings` tools and return the menu directly in your IDE.
   ...
   ![Copilot menu response](../Screenshot/assets/lab03/task2.8.png)

9. Place an order through Copilot:

   `"Order one burger for me — surprise me with any burger you think is good"`

   >**Note:** You will be asked to provide a `userId` for the order. You must use the same string from your previous task - **step 11**.

   ![Copilot order prompt](../Screenshot/assets/lab03/task2.9.1.png)

   Watch Copilot decide which burger to pick, call `get_burgers` to choose one, then call `place_order` with your user context. 
   
   If you are prompted - select `allow in this session` to let Copilot call the MCP tools.

   ![Copilot place order](../Screenshot/assets/lab03/task2.9.2.png)
...
10. Switch back to your **Burger Web App** browser tab. The order placed through Copilot should appear on the live orders dashboard — confirming that the same MCP server powers both the web agent and your IDE assistant.
    ...
    ![Order from Copilot on dashboard](../Screenshot/assets/lab03/task2.10.png)

11. Check your order history through Copilot:

    `"Show me my recent orders"`

    ![Copilot order history](../Screenshot/assets/lab03/task2.11.png)

<validation step="validate-copilot-mcp-integration" />

> **Congratulations** on completing Lab 03! You have tested MCP tools directly via the Inspector and integrated the same server into GitHub Copilot — demonstrating that one MCP server can power multiple AI clients simultaneously, with no changes to the server itself.
